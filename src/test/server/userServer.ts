...
export const handlers = [
  rest.post('http://localhost:5137/api/auth/login', async (req, res, ctx) => {
  ...
  rest.get('http://localhost:5137/api/auth/profile', (req, res, ctx) => {
  ...
  rest.get('http://localhost:5137/api/user/', (req, res, ctx) => {
    return res(ctx.json(usersData));
  }),

  rest.post('http://localhost:5137/api/user/', async (req, res, ctx) => {
  ...
  rest.patch('http://localhost:5137/api/user/:id', async (req, res, ctx) => {
    const { id } = req.params;
    const userId = id;
